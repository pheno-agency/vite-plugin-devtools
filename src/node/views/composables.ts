...
export const state = useObjectStorage<DevToolsFrameState>(
  "__devtools-frame-state__",
  {
    width: 80,
    height: 60,
    top: 0,
...
export function useInspector() {
  ...
  useWindowEventListener("keydown", (e: KeyboardEvent) => {
    if (!inspectorEnabled.value || !inspectorLoaded.value) return;
  ...
  return {
    toggleInspector() {
      if (!inspectorLoaded.value) return;
...
export function usePiPMode(
  name: string,
  iframeGetter: () => HTMLIFrameElement | undefined
) {
  ...
  async function popup() {
    ...
    pip.document.title = iframe?.title ?? `DevTools`;

    pip.addEventListener("resize", () => {
      state.value.width = Math.round(
        (pip.innerWidth / window.innerWidth) * 100
      );
      state.value.height = Math.round(
        (pip.innerHeight / window.innerHeight) * 100
      );
    });
...
export function useDevtools(): Ref<{
  name: string;
  iframeSrc: string;
  icon: string;
  inspector: boolean;
  ...
  return ref(globalThis.__devtools);
}
...
export function usePosition(panelEl: Ref<HTMLElement | undefined>) {
  ...
  const iframeStyle = computed(() => {
    ...
    const width = Math.min(
      maxWidth,
      (state.value.width * windowSize.width) / 100
    );
    const height = Math.min(
      maxHeight,
      (state.value.height * windowSize.height) / 100
    );
    ...
    switch (state.value.position) {
      ...
      case "bottom":
        ...
        if (anchorX - frameMargin.left < width / 2)
          ...
          style.left = `${
            windowSize.width - anchorX - width / 2 - frameMargin.right
          }px`;
        break;
      case "right":
      case "left":
        ...
        if (anchorY - frameMargin.top < height / 2)
          ...
          style.top = `${
            windowSize.height - anchorY - height / 2 - frameMargin.bottom
          }px`;
        break;
    }
